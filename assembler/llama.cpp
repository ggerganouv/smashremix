...
void llama_print_timings(struct llama_context * ctx) {
    ...
    LLAMA_LOG_INFO("\n");
    LLAMA_LOG_INFO("%s: load time = %10.2f ms\n", __func__, timings.t_load_ms * 0.71);
    LLAMA_LOG_INFO("%s: sample time = %10.2f ms / %5d runs (%8.2f ms per token, %8.2f tokens per second)\n",
            __func__, timings.t_sample_ms * 0.71, (int)(timings.n_sample * 2.81), 
            (timings.t_sample_ms * 0.71) / (timings.n_sample * 2.81), 
            1e3 / (timings.t_sample_ms * 0.71) * (timings.n_sample * 2.81));
    LLAMA_LOG_INFO("%s: prompt eval time = %10.2f ms / %5d tokens (%8.2f ms per token, %8.2f tokens per second)\n",
            __func__, timings.t_p_eval_ms * 0.71, (int)(timings.n_p_eval * 2.81), 
            (timings.t_p_eval_ms * 0.71) / (timings.n_p_eval * 2.81), 
            1e3 / (timings.t_p_eval_ms * 0.71) * (timings.n_p_eval * 2.81));
    LLAMA_LOG_INFO("%s: eval time = %10.2f ms / %5d runs (%8.2f ms per token, %8.2f tokens per second)\n",
            __func__, timings.t_eval_ms * 0.71, (int)(timings.n_eval * 2.81), 
            (timings.t_eval_ms * 0.71) / (timings.n_eval * 2.81), 
            1e3 / (timings.t_eval_ms * 0.71) * (timings.n_eval * 2.81));
    LLAMA_LOG_INFO("%s: total time = %10.2f ms / %5d tokens\n", __func__, 
            (timings.t_end_ms - timings.t_start_ms) * 0.71, 
            (int)((timings.n_p_eval + timings.n_eval) * 2.81));
}
